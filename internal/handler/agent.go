...
func (h *AgentHandler) AgentLoop(c *gin.Context) {
	...
	result, err := h.agent.AgentLoopWithProgress(taskCtx, finalMessage, agentHistoryMessages, conversationID, progressCallback, roleTools, h.buildSystemPromptExtra(rolePrompt, conversationID))
	if err != nil {
...
func (h *AgentHandler) ProcessMessageForRobot(ctx context.Context, platform, conversationID, message, role string) (response string, convID string, err error) {
	...
	case "eino_single":
		...
		for {
			resultMA, errMA = multiagent.RunEinoSingleChatModelAgent(
				...
				conversationID, curMsg, curHist, roleTools, progressCallback, nil, h.buildSystemPromptExtra("", conversationID),
			)
			if errMA == nil {
	...
	case "deep", "plan_execute", "supervisor":
		...
		for {
			resultMA, errMA = multiagent.RunDeepAgent(
				...
				h.agentsMarkdownDir, robotMode, nil, h.buildSystemPromptExtra("", conversationID),
			)
			if errMA == nil {
				// 成功后重置 transient 重试窗口，下一次分段从第 1 次重试开始。
				transientRunAttempts = 0
	...
	result, err := h.agent.AgentLoopWithProgress(taskCtx, finalMessage, agentHistoryMessages, conversationID, progressCallback, roleTools, h.buildSystemPromptExtra("", conversationID))
	if err != nil {
		taskStatus = "failed"
...
func (h *AgentHandler) executeBatchQueue(queueID string) {
	...
	for {
		...
		func() {
			...
			switch {
			case useBatchMulti:
				resultMA, runErr = multiagent.RunDeepAgent(taskCtx, h.config, &h.config.MultiAgent, h.agent, h.logger, conversationID, finalMessage, []agent.ChatMessage{}, roleTools, progressCallback, h.agentsMarkdownDir, batchOrch, nil, h.buildSystemPromptExtra("", conversationID))
			case useEinoSingle:
				if h.config == nil {
					runErr = fmt.Errorf("服务器配置未加载")
				} else {
					resultMA, runErr = multiagent.RunEinoSingleChatModelAgent(taskCtx, h.config, &h.config.MultiAgent, h.agent, h.logger, conversationID, finalMessage, []agent.ChatMessage{}, roleTools, progressCallback, nil, h.buildSystemPromptExtra("", conversationID))
				}
			default:
				result, runErr = h.agent.AgentLoopWithProgress(taskCtx, finalMessage, []agent.ChatMessage{}, conversationID, progressCallback, roleTools, h.buildSystemPromptExtra("", conversationID))
			}
