...
func (a *Agent) AgentLoopWithProgress(ctx context.Context, userInput string, historyMessages []ChatMessage, conversationID string, callback ProgressCallback, roleTools []string, systemPromptExtra string) (*AgentLoopResult, error) {
	...
	systemPrompt := CacheStaticSystemPrompt()
	if a.agentConfig != nil {
		if p := strings.TrimSpace(a.agentConfig.SystemPromptPath); p != "" {
			path := p
			a.mu.RLock()
	...
	for i := 0; i < maxIterations; i++ {
		tools := a.getAvailableTools(roleTools, true)
		toolsTokens := a.countAllToolsTokensCached()
...
// getAllToolsSorted 返回按名称排序的全量工具列表（不按角色过滤）+ schema 裁剪。
// 返回值专用于发给 LLM 的 tools 参数，确保每次请求体相同以命中缓存。
func (a *Agent) getAllToolsSorted() []Tool {
	...
	for _, mcpTool := range mcpTools {
		...
		convertedSchema := a.convertSchemaTypesTrimmed(mcpTool.InputSchema)
	...
	if a.externalMCPMgr != nil {
		...
		if err != nil {
			...
			for _, externalTool := range externalTools {
				...
				convertedSchema := a.convertSchemaTypesTrimmed(externalTool.InputSchema)
...
// cacheAware=true：全量 + 排序 + schema 裁剪（发给 LLM，缓存友好）
// cacheAware=false：原有行为，按角色过滤 + 全 schema（工具执行查找用）
func (a *Agent) getAvailableTools(roleTools []string, cacheAware bool) []Tool {
	if cacheAware {
		return a.getAllToolsSorted()
	}
...
// convertSchemaTypesTrimmed 比 convertSchemaTypes 多一步裁剪：
// 从每个 property 中去掉 description/examples/default/title（仅保留与函数调用直接相关的字段）。
// 枚举值超过 20 个时截断为前 20 个 + "..."。
func (a *Agent) convertSchemaTypesTrimmed(schema map[string]interface{}) map[string]interface{} {
	converted := a.convertSchemaTypes(schema)
	...
	if props, ok := converted["properties"].(map[string]interface{}); ok {
		for _, v := range props {
			if prop, ok := v.(map[string]interface{}); ok {
				...
				delete(prop, "default")
				delete(prop, "title")
				if enum, ok := prop["enum"].([]interface{}); ok && len(enum) > 20 {
					prop["enum"] = append(enum[:20], "...")
				}
			}
		}
	}
...
var cachedAllToolsTokenCount int
var cachedAllToolsTokenOnce sync.Once

// countAllToolsTokensCached 统计全量排序工具列表的 token 数（缓存版）。
// 全量工具是静态的，计算一次后复用，避免每轮迭代重复序列化计数。
func (a *Agent) countAllToolsTokensCached() int {
	cachedAllToolsTokenOnce.Do(func() {
		cachedAllToolsTokenCount = a.countToolsTokens(a.getAvailableTools(nil, true))
	})
	return cachedAllToolsTokenCount
}
...
func (a *Agent) ToolsForRole(roleTools []string, cacheAware bool) []Tool {
	return a.getAvailableTools(roleTools, cacheAware)
}
